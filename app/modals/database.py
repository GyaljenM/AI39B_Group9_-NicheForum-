...
class Database:
    ...
    def create_tables():
        ...
        try:
            db.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS username VARCHAR(100) NOT NULL UNIQUE AFTER name")
        except: pass
        try:
            db.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS bio TEXT AFTER password")
        except: pass
        try:
            db.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS profile_picture VARCHAR(255) AFTER bio")
        except: pass
        ...
        if not admin:
            ...
            db.execute(
                ...
                ("Admin", "admin", "admin@admin.com", generate_password_hash("admin123"), "admin"),
            )
